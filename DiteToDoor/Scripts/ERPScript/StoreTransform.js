...
$(function () {
    ...
    $("#submit").click(function () {
        ...
        $("#OrdersItems tbody tr").each(function () {
            ...
            var item = {
                StoreTransformNo: $('.StoreTransformNo', this).val(),
        ...
        if (document.getElementById("ToStoreId").selectedIndex == 0) {
            toastr.warning('اختر المخزن المحول إليه ...!')
            isValid = false;
        }
        if (document.getElementById("FromStoreId").selectedIndex == 0) {
            toastr.warning('اختر المخزن المحول منه ...!')
            isValid = false;
        }
        ...
        if (isValid) {
            var data = {
                StoreTransformNo: $("#StoreTransformNo").val(),
                StoreTransformDate: $("#StoreTransformDate").val(),
                PermissionType: $("#VendorsList").val(),
                PermissionNo: $("#PermissionNo").val(),

                Notes: $("#Notes").val(),

                ToStoreId: $("#ToStoreId").val(),
                FromStoreId: $("#FromStoreId").val(),
                ItemNo: $("#RowsCount").text(),
            ...
            $.ajax({
                type: 'POST',
                url: '/StoreTransform/AddStoreTransform',
...
$("#update").click(function () {
    ...
    if (isValid) {
        ...
        $.ajax({
            type: 'POST',
            url: '/StoreTransform/updateStoreTransform',
            data: JSON.stringify(data),
            contentType: 'application/json',
            success: function (data) {
                if (data.status) {
