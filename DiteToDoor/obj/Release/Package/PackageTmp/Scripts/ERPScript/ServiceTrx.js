...
$(function () {
    ...
    $("#addRowService").click(function () {
        ...
        if (isValid) {
            var productID = document.getElementById("Service").value;
            ...
            $('#ItemNumber,#Service,#Qty', $newRow).attr('disabled', false);
            $('#ItemNumber,#Service,#Qty', $newRow).removeAttr('id');
    ...
    $("#addEdit").click(function () {
        ...
        if (isValid) {
            ...
            $('#Service,#Qty', $newRow).attr('disabled', false);
            $('#Service,#Qty', $newRow).removeAttr('id');
            $('span.error', $newRow).remove();
            $("#OrdersItems").append($newRow[0]).remove("#ItemCategoriesList");
            $("#OrdersItems #GroupHide").hide();
            $("#OrdersItems #ItemCode").show();
          ...
          /*  $lastTr.find('.UnitList').select2();*/ // Re-instrument original row
            ...
            $.get("/PurchaseINV/GetItemList", { ID: 0 }, function (data) {
                $("#ItemList").empty();
                $("#ItemList").append("<option> اختر الخدمة  </option>")
                $.each(data, function (index, row) {
            ...
            $.get("/PurchaseINV/GetItemGroupList", function (data) {
                $("#ItemCategoriesList").empty();
                $("#ItemCategoriesList").append("<option> اختر الخدمة  </option>")
                $.each(data, function (index, row) {
    ...
    $("#submit").click(function () {
        ...
        if (isValid) {
            ...
            $.ajax({
                type: 'POST',
                url: '/ServicesTrx/AddServicesTrx1',
